...
\begin{table}[htbp]
  \centering
  \caption{Why the baseline had to be re-measured in the adapter's own numeric precision (adapter always bf16; the unmatched row compares it against the 4-bit deployment baseline). The unmatched comparison understates the regression by a factor of 21.}
  ...
  \begin{tabular}{lrrrl}
    ...
    \midrule
    unmatched: bf16 vs 4-bit & 0.8246 & 0.8257 & -0.0011 & ``no measurable harm'' \\
