...
% NOTE: future work lives in Ch.~\ref{ch:future-work} (per the
...
can current open-weights document \acp{VLM}, deployable on hardware a German tax-consulting
company already owns, convert business-to-business invoices into the legally grounded
\ac{EN}~16931 form --- and which stage of such a pipeline limits what is achievable? The
professional-secrecy regime of \S~203~\ac{StGB} makes the local constraint an operational
premise, not a preference (Section~\ref{sec:motivation}), and the thesis asked what is
...
(Section~\ref{sec:results-heldout}). The error profile leans the right way for the domain:
...
the design (Section~\ref{sec:design-honest-null}) showing up in the measurements. The cost
...
paper (Section~\ref{sec:lim-corpus}).
...
(Section~\ref{sec:results-baseline}); and in the per-miss attribution --- taken with the
...
(Section~\ref{sec:results-attribution}). Effort spent
...
the rate of values invented for absent fields (Section~\ref{sec:results-adaptation}).
...
(Section~\ref{sec:validity-confound}). None of these defects announced themselves; every one
...
unevaluated, never quietly dropped (Section~\ref{sec:unevaluated}). No cloud comparison
...
different evidence. Every number reported above was produced on hardware a company plausibly owns:
nothing needed more than a single 24\,\ac{GB} graphics processor, running open-weights models
under the author's control at every step. The laptop floor, whose memory ceiling caps
the reader at roughly half of native page resolution, has a measured structuring stage but an
unmeasured reading quality (Section~\ref{sec:method-repro}). The delivered pipeline is open-weights,
...
(Section~\ref{sec:lim-hardware}). Within those boundaries, the claims are as
strong as the committed evidence behind them --- every measured number in this manuscript
derives from the archived measurement artefacts --- and no stronger.

\section*{Outlook}
\addcontentsline{toc}{section}{Outlook}

The path forward is ordered by the evidence, not by ambition, and
Chapter~\ref{ch:future-work} states it in full: improve capture and
