...
(Section~\ref{sec:contributions}): the software that runs the pipeline, produces the numbers, and
...
have made the cohort survey of Section~\ref{sec:results-preliminary} impossible, because several of
...
restarting it therefore costs only the cell in flight. On this hardware this is a necessity
rather than a convenience: a full sweep does not reliably fit into one uninterrupted session, and without
...
The adaptation study of Section~\ref{sec:results-adaptation} had to move off the local machine, for
reasons Section~\ref{sec:method-repro} states. That created a risk worth naming: two training paths
...
regenerate every measurement table and chart in this manuscript, and build this manuscript.
...
discoverable, reviewable in a diff, and identical for every invocation. The measurement
tables and charts in Chapters~\ref{ch:results} and~\ref{ch:measurement-validity} are
generated by one such target from committed evaluation output, which is what makes the prohibition on transcribing
...
\begin{figure}[htbp]
  \centering
  \input{figures/app-surfaces.tex}
...
held-out answer key of Section~\ref{sec:method-heldout-gt}, and it is the clearest case in this
...
target class (Section~\ref{sec:method-repro}). One external
...
company's information-technology department. The system is operated from a source checkout by
someone who can read a build file.

That is a scope decision, not an omission, and the reasoning is the same as in
Section~\ref{sec:design-scope}. Packaging serves adoption; nothing in the research questions of
Section~\ref{sec:research-questions} depends on it, and the effort it would have consumed went into
...
listed in Section~\ref{sec:fw-production}, and the prototype's review surface is the part of it that
