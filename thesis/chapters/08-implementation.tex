...
(Chapter~\ref{ch:results}). This chapter describes the artefact: the software that runs the
pipeline, produces the numbers, and lets a human look at what the system did.

It is deliberately short on inventory and long on the few structural choices that had
methodological consequences. Three of them recur: configuration is data rather than code,
every experiment is resumable and records its own provenance, and every research surface
recomputes what it displays from the stored per-field outcomes rather than showing a summary
...
that schema do real work. Unknown keys are rejected rather than ignored, so a misspelt setting
...
malformed configuration costs a second rather than the several minutes it takes to read
...
dozen documents --- executed on a laptop. Three properties follow from that setting rather than
...
number in this thesis traceable to the run that produced it rather than to a note.
...
once per epoch --- rather than implemented by hand, so the executed rule cannot drift from the
...
actually sits: the two largest files cover the scoring rules and the typed schema with its
repair pass, and the value normalisers and reference extraction follow closely, because those
are the components whose defects corrupt results silently rather than loudly. Several
tests exist specifically to pin down invariants that were violated once --- that the field
registry has the size the reported figures assume, that normalisation is applied to both sides
of a comparison, and that no concrete value from the corpus appears inside a prompt.

Tests that require the corpus, or that require Apple-Silicon inference, skip themselves by
predicate rather than being excluded by a marker or deleted. The same suite therefore runs
locally with the corpus present, locally without it, and in continuous integration, with the
skips visible in the output rather than hidden in configuration.

Continuous integration runs on every push and every pull request: linting, formatting,
type-checking with a cleared cache, and the full suite. The cleared type-check cache is a
response to a specific incident --- a warm cache silently masked an import error after a
configuration change --- and is documented as such rather than being a general caution.
...
numbers into the manuscript enforceable rather than aspirational.
...
the same tracking store the sweeps write to, and has five surfaces.
...
outcomes at display time rather than reading a summary number that was written earlier. The
...
change. On the target hardware --- one laptop, no accelerator, no network dependency on the
inference path --- a checkout runs every experiment reported in this thesis. One external
...
That is a scope decision rather than an omission, and the reasoning is the same as in
