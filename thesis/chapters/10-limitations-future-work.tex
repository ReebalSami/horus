...
The system was developed inside, and is delivered for, a single laptop with 16\,\ac{GB} of
unified memory. That is a narrower claim than \emph{runs locally}: the privacy premise is
on-premises processing, which would also permit a workstation with a discrete accelerator, and
such a machine would likely support a larger reading model than the one selected here.

The envelope bound training and the heavier measurement passes, not delivered inference. The
adaptation runs exceeded it and were moved to a rented accelerator, together with the
precision-matched adaptation evaluations and the native-resolution reading passes
(\S\ref{sec:method-repro}); the local-only property therefore holds for the delivered
pipeline, but not for the process that produced its adapters or its final measurement record.
Since neither the adapters nor the measurement harness is part of the delivered system, the
delivered system remains fully local --- but a firm wishing to adapt the pipeline to its own
documents, or to reproduce the full measurement, on its own hardware could not do so within
this envelope.
