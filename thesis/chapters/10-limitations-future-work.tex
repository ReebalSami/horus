...
(\S\ref{sec:results-reader}). \emph{The structuring model was not chosen at all.} It was fixed
...
The hardware claim is two-tier (\S\ref{sec:method-repro}): a \emph{target class} --- a
...
checkpoints under the author's control (\S\ref{sec:method-repro}). The local-only property
therefore holds for the delivered pipeline at both tiers --- open-weights end to end, no
network call on any inference path, both models resident within 16\,\ac{GB} --- and a firm at
the target class could also adapt the pipeline and reproduce the full measurement record on
premises. A firm equipped only at the floor could run the pipeline but not adapt it or
...
as the dominant real-world cost. The channel comparison of \S\ref{sec:results-heldout} shows
...
and is specified in \S\ref{sec:fw-envelope}. It was not taken within the scope of this thesis
because the experiment track was frozen in favour of the writeup
(\S\ref{sec:design-scope}), and recording the gap was judged better than an unplanned run
...
could have revealed it (\S\ref{sec:validity-open}). The affected cells are all dates and
...
unlike the answer-key work (\S\ref{sec:lim-cloud-gt}) this measurement is itself deployable
...
scorer, same greedy decoding. The result would convert \S\ref{sec:lim-hardware} from a stated
gap into a measured quantity --- the accuracy cost of the floor relative to the target class,
on real documents --- which is the number a firm equipping below the target class would
...
built to support the evaluation, not a firm's daily workflow, and the difference between
