...
can current open-weights document \acp{VLM}, running entirely on hardware a German tax
...
premise rather than a preference (\S\ref{sec:motivation}), and the thesis asked what is
...
perfect transcripts while the delivered pipeline, differing only in that the transcript
...
unevaluated rather than quietly dropped (\S\ref{sec:lim-scope}). No cloud comparison
was run. The held-out corpus is small and single-annotator, so its figures establish a
level and a direction rather than precise values; the structuring model was never
competitively selected, unlike the reader; and everything measured concerns German and
European \ac{B2B} invoices, nothing else. Within those boundaries, the claims are as
strong as the committed evidence behind them --- every number in this manuscript is
generated from the archived measurement artefacts rather than transcribed by hand ---
and no stronger.

\section*{Outlook}
\addcontentsline{toc}{section}{Outlook}

The path forward is ordered by the evidence rather than by ambition, and
