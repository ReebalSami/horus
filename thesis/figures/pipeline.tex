...
%% two transcript-level measurement instruments attach.
%%
...
\begin{tikzpicture}[
  ...
  %% Both transcript-level instruments act on the transcript, which is why they share one
  %% box and one attachment point. The labels use the instrument names of the methodology
  %% chapter (Section sec:method-attribution): instrument two is per-miss findability,
  %% instrument three the perfect-text ceiling; instrument one (cluster-level re-scoring)
  %% attaches to stored predictions, not to the transcript, and is not drawn here.
  ...
  \node[
    ...
    \textbf{The transcript is where the two transcript-level measurement instruments
    attach:}\\[2pt]
    \textbf{Instrument two, per-miss findability:} for each individual miss, ask whether
    the value was present in it at all\\[1pt]
    \textbf{Instrument three, the perfect-text ceiling:} replace it with text rendered
    from the ground truth --- this removes the reading stage from the measurement and
