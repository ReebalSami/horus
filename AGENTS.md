...
- `make app` — Streamlit observability dashboard over the local MLflow runs + saved transcripts + CII ground truth. Three read-only research surfaces (Overview / Invoice Explorer / Approach Comparison; ADR-036, #103) plus two bounded WRITE surfaces: *Extract an Invoice* (live demo; ADR-039) and *Ground Truth Sign-off* (the ADR-062 escalation queue — reads `data/self-collected/_review/manifest.json`, writes `_promoted/<id>.gt.json`; never touches `gt/`). No model inference outside the demo page. Entry `app/Home.py`; pages in `app/views/`; data layer in `app/data/` (`APP_PORT=<n>` to override 8501)
- `make thesis` — build the thesis PDF via latexmk (pdflatex + biber) from `thesis/` → `thesis/_build/main.pdf`; `make thesis-clean` removes build artifacts. Self-contained LaTeX (needs TeX Live/MacTeX, **not** uv). FH Wedel template adapted to English; biblatex `authoryear` + biber (switched from the template default `alphabetic` in the 2026-08-16 review pass, per the first examiner's established preference and the Richtlinie's `Kurzbelegmethode`). See ADR-055 + `thesis/README.md`.
