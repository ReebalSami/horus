...
## Addendum 3 — measurement-venue overclaim found by the author, corrected (2026-08-16, same day)

**The finding (grade-blocking class; author-caught).** The author questioned the abstract's "hosted entirely on a 16 GB Apple-silicon laptop" against the memory of AWS GPU sessions. Verification against the committed record confirmed the doubt:

- `scripts/gpu/README.md` §5B + `scripts/transcribe_heldout.py` docstring: the **held-out reading pass — the reading stage of the headline 0.88 result — ran on a rented A10G at bf16, native resolution** (observed 2026-08-04: 39 invoices / 58 pages / 0 errors / ≈ $0.45). The script itself says a local run is "wiring verification only" because the MPS `max_pixels` cap reads A4 at ≈ half resolution.
- `eval/structurer-lora-2x2-results.md`: all six 2×2 grid numbers are "bf16, CUDA (A10G 23 GB)".
- The reader bake-off (#55) ran at full precision on the same box (M1 disqualified every full-precision candidate: 35 GiB Metal buffers, 4-bit German-text corruption, 49 min/page bf16).
- The thesis's own `tab:sealed-val-arms` prints "bf16 / CUDA" arm labels — internally contradicting the prose.

Three passages claimed otherwise and were corrected: ch.5 §method-repro "All inference reported in this thesis runs on a single Apple M1 Pro laptop … Delivered inference remains entirely local; only training moved" → rewritten as deployment-envelope claim + three recorded classes of rented-CUDA pass (training; precision-matched adaptation evals; native-resolution reading passes) with the confound rationale for each; ch.10 §lim-hardware "All inference was developed and measured inside a single laptop" + "The envelope bound training, not inference" → corrected to development/delivery vs measurement split; ch.9 §disc-local "broke the premise, once" → now counts both recorded breaks (cloud GT channels; rented transcription box — author-controlled instance, a different privacy class than a third-party service, but off-premises). Abstract: "hosted entirely on a 16 GB Apple-silicon laptop … both running locally" → "sized for commodity hardware --- a laptop with 16 GB of memory" + "the delivered system runs both stages on-premises" (design-envelope and delivery claims, both true).

**Lesson**: the venue split was fully documented in the repo (runbook, script docstrings, eval reports, even a table's arm labels) yet the prose absolutism survived two review passes because every review traced *numbers*, not *venue metadata*. Added to the integration-review prompt's number-consistency dimension by this precedent.

**Also this pass**: title-page subtitle reshaped to `HORUS` + `(Hybrid OCR-free Reading and Understanding System)` on the next line (author decision; trailing "System" after HORUS dropped — the visible backronym would have made it "…Understanding System System"). The ch.1 OCR-free terminology note was verified 100% against primary sources: Donut (Kim et al., ECCV 2022) coined "OCR-free" exactly as the architectural term the note uses; olmOCR-2 confirmed as a Qwen2.5-VL-7B fine-tune reading pages "in a single pass" (Ai2 blog + model card) — task-named, architecturally OCR-free, precisely the note's case; "several … in the evaluated cohort" grounded by ADR-009 (olmOCR-2, DeepSeek-OCR-2, PaddleOCR-VL, GLM-OCR), with "olmOCR-2 among them" now named inline since the eliminated cohort members are not enumerated in the manuscript. One nuance recorded: PaddleOCR-VL as shipped is a two-stage system (PP-DocLayoutV2 layout detector + 0.9B VLM recognizer) — not single-pass, though its character recognition is the VLM; the note's claims are about OUR pipeline and about naming, so it stands. Verified after all changes: clean rebuild at 128 pp, zero unresolved references/citations, worst overfull 3.22 pt (pre-existing), title + abstract pages visually re-inspected.
