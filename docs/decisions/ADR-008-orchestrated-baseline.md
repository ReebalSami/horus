...
H2 is locked-a-priori per v2 §4.1 (no HARKing — hypotheses fixed before seeing test results). The ADR does **not** lock the threshold parameters of H2 (the *X* and *Y* percentage-point quantifications); those belong in issue #17's first-Säring-meeting hypothesis-set deliverable per v2 §4.1 + brainstorm §4. This ADR locks the *tooling*; #17 locks the *hypothesis specification*.
...
| **Skip orchestrated baseline entirely** | n/a | n/a | n/a | Issue #11 alternative #4 (AC explicit permission) | **Rejected.** H2 is locked-a-priori per v2 §4.1 + §6 (no HARKing — hypotheses fixed before testing). Skipping forfeits the directional-flip test that is a Layer-1 sub-question of the thesis. Säring meeting #17 needs concrete instruments on **both** arms (single-shot via cohort ADR #14; orchestrated via this ADR). Defer-and-skip is the worst path for scientific correctness. |
...
- **Issue #17** — Säring agenda; H2 hypothesis-set deliverable will cite this ADR + cohort ADR #14 as tooling enablers.
...
1. **Docling installs + runs end-to-end on M1 Pro hardware**, producing **substantively correct extraction** of the HORUS ZUGFeRD invoice. Successfully extracted seller block (HORUS Test Seller GmbH / Teststraße 1 / 20095 Hamburg / DE / USt-ID: DE123456789), buyer block (HORUS Test Buyer GmbH / DE), full line-item table with header row + position row + subtotal/VAT/Brutto/Zahlbar rows, and the document date. **This validates ADR-008's primary-tool choice (Docling) as a working orchestrated baseline on HORUS-style invoices.** It is also a sharp contrast to ADR-007's smoke evidence on Granite-Docling-258M, which produced 0 correct extractions across 4 prompts on the same input PDF — reinforcing v2 §6 H2's *clean-document* arm framing in the **opposite direction** to Berghaus 2025's prior (Berghaus measured Docling at 64.03% on Scanned Invoices vs cloud 92.71%, but on a *clean fpdf2-rendered* HORUS invoice Docling appears to perform substantially better than the 64% prior would suggest — a finding pilot #13 should formally test). Caveat: this is N=1; pilot #13 will run on 10–50 invoices to produce statistically meaningful evidence.
...
3. **Forward-pointer evidence for pilot #13** — the smoke surfaces three substantive observations that pilot #13 should treat as testable claims rather than smoke artefacts:
   - **The invoice number ("Nr. HORUS-SMOKE-001") is missing from the extracted output.** This is a W=3 critical field per v2 §5.2 (required for Vorsteuerabzug). Either (a) the fpdf2 layout placed "Nr. HORUS-SMOKE-001" in a region Docling's reading-order classifier dropped, or (b) PP-OCR confidence thresholds rejected the small "Nr." prefix line. Either way, this is a **measurable failure mode** for pilot #13's compliance-aware F1 — an orchestrated-pipeline-specific weakness on Rechnungsnummer extraction that ADR-008's H2-test instrument has empirical evidence of, ahead of full pilot evaluation.
   - **Reading order placed "Datum: 11.05.2026" at the *bottom* of the markdown output**, not adjacent to the "RECHNUNG" heading where fpdf2 rendered it visually. This is a **layout-reading-order failure mode** that pilot #13 should record explicitly as a per-field accuracy line item — Docling's layout classifier may struggle with multi-block top-of-page layouts on fpdf2-style invoices.
   - **Structural counts (`pages=2 texts=9 tables=1 pictures=0`) report 2 pages on a 1-page fpdf2 invoice.** Likely cause: Docling counts the embedded ZUGFeRD CII XML attachment as a separate "page". Pilot #13 should pre-flight-check whether the orchestrated pipeline correctly disambiguates visual pages from PDF/A-3 attachments — relevant for any per-page-aggregated metrics.

   None of these three findings invalidate ADR-008's tool choice; they are **forward-pointer evidence** that the pilot should systematically test, not smoke-time defects to address before merge.
